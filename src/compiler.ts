...
export default class Compiler {
  ...
  constructor(code: string) {
    const rules = new Map([
      ...
      [
        TokenType.Minus,
        new ParseRule(this.unary, this.binary, Precedence.Term),
      ],
  ...
  private binary() {
    ...
    switch (operatorId) {
      ...
      case TokenType.Minus:
        return this.emit(OpCode.Subtract);
      case TokenType.Star:
        return this.emit(OpCode.Multiply);
      case TokenType.Slash:
        return this.emit(OpCode.Divide);
    }
  }
  ...
  private parsePrecedence(prec: Precedence) {
    ...
    if (prefixRule != null) {
      ...
      return this.error(`Expected expression after ${this.curr.id}`);
    }
